...
###Some vitualiztion method 
```{r}
library(plotly)
set.seed(100)
d <- Proj_1 [sample(nrow( Proj_1 ), 5000), ]
plot_ly(x = d[,4], y = d[,11], 
        mode = "markers", color = d[,3], size = d[,4])

```

```{r}

d <- Proj_1 [sample(nrow( Proj_1 ), 50000), ]
plot_ly( x = d[,4], y = d[,11], mode = "markers",
        color = as.factor(d[,6]), colors = "Set2")
